...
{
 $content['videos']= $db1->db_query("SELECT * FROM videos WHERE fname like '%.m3u8%' and sub = 'false' and name like".$db1->GetSQLValueString('%'.$_GET['psearch'].'%', "text")."",array (
    'name','image','id','views','fname'
),20,0,true);
}
else{
$content['videos']= $db1->db_query("SELECT * FROM videos WHERE (`type` ='yt' and sub = 'false') OR ( fname like '%.m3u8%' and sub = 'false')",array (
    'name','image','id','views','fname'
),20,0,true);}
...
@$smarty->assign('page_title',$title['vod']);
