		...
		<div class="container">
			...
			<div class="col-md-9 col-sm-8">
				<div align="center" style="margin-bottom:10px;">{$ads.side.0.code}</div><div class="product-category">
					<h4>Categories {if $cate.0.is_empty == 'false'}: {$cate.0.name}{else}{/if}</h4>
						
				</div>
  {if $files.0.is_empty == 'false'}
  <ul class="product-main row">{foreach $files as $items}<li class="col-md-4 col-sm-6 col-xs-6">
				...
				</ul>{include file='paginations.tpl'}{else}<h3>No products found</h3>{/if}
				</div>
			<!-- col-md-9 /- -->
			</div>
		</div><!-- container /- -->
