...
<div id="product-section" class="product-section shop">
		...
		<div class="container">
			...
			<div class="col-md-9 col-sm-8">
				...
				{if $videos.0.is_empty == 'true'}<h3>No Videos found</h3>{else}<ul class="product-main row">
				  {foreach $videos as $items}
				  <li class="col-md-4 col-sm-6 col-xs-6" style="min-height:180px;">
					<div class="product-box">
						<a title="Product" href="{$site_root}/watch-vod.php?id={$items.id}">
						<img src="{$items.image}" alt="{$items.name}"  class="tr_all_hover"></a>
						<div class="product-detail">
							<a title="Product Name" href="{$site_root}/watch-vod.php?id={$items.id}" class="product-title">{$items.name|truncate:40}</a></div>
					</div>
					</li>{/foreach}
