			...
			<div class="page_content_offset">
				<div class="container">
                 ...
                 <b> {$items.name}</b> </a> </li>{/foreach}
						...
						{$ads.top.0.code}
							<h2 class="tt_uppercase color_dark m_bottom_10 heading5 animate_ftr">Free Streams</h2>
					        <!--products-->
					
							<section class="color_dark m_bottom_10 heading5 animate_ftr"><div class="shadow bg-white mTop-30 frameLR" id="container1">
                
                {if $streams.0.is_empty == 'true'}
                <h4>No free streams added</h4>
                {else}{foreach $streams as $items}<div class="free">
                  <h3>{$items.name}</h3>
                  <strong><a href="http://{$smarty.server.HTTP_HOST}{$site_root}/iptv.php?list={$items.m3u_name}" target="_blank">http://{$smarty.server.HTTP_HOST}{$site_root}/iptv.php?list={$items.m3u_name}</a></strong>
                  </div>
                {/foreach}{/if}</div></section>
						</section>
					</div>
			  </div>
			</div>
