			...
			<div class="page_content_offset">
				<div class="container">
                 ...
                 <b> {$items.name}</b> </a> </li>{/foreach}
						...
						{$ads.top.0.code}
							<h2 class="tt_uppercase color_dark m_bottom_10 heading5 animate_ftr">Product categories</h2>
							<!--products-->
							<section class="products_container a_type_2 category_grid clearfix m_bottom_25">
						
						{if $videos.0.is_empty == 'true'}<h3>No  Products found.</h3>
						{else} {foreach $files as $items}  
						  <div class="product_item rated w_xs_full">
									<figure class="r_corners photoframe animate_ftb long type_2 t_align_c shadow relative">
										<!--product preview-->
										<a href="{$site_root}/product/{$items.display}" class="d_block relative pp_wrap m_bottom_15">
										<img src="{$site_root}/prod/{$items.image}" alt="{$items.name}"  class="tr_all_hover">
											
					          </a>
										<!--description and price of product-->
										<figcaption>
											<h5 class="m_bottom_10"><a href="{$site_root}/product/{$items.display}" class="color_dark">{$items.name}</a></h5>
											<div class="clearfix">
												<!--rating-->
											  <p class="scheme_color f_size_large m_bottom_15">{$curency}{$items.price}</p>
										  </div>
										  <button class="button_type_4 bg_scheme_color r_corners tr_all_hover color_light mw_0 m_bottom_15"  onClick="add_basket({$items.id})">Add to Cart</button>
											</figcaption>
									</figure>
								</div>{/foreach}
							</section>
