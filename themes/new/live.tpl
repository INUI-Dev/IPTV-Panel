			...
			<div class="page_content_offset">
				<div class="container">
                 ...
                 <b> {$items.name}</b> </a> </li>{/foreach}
						...
						{$ads.top.0.code}
							<h2 class="tt_uppercase color_dark m_bottom_10 heading5 animate_ftr">Videos</h2><form id="form1" name="form1" method="get" action="">Filter : 
						...
						{foreach $videos as $items}  
						  ...
						  <div class="product_item rated w_xs_full">
						    <figure class="r_corners photoframe animate_ftb long type_2 t_align_c shadow relative">
						      <!--product preview-->
						      <a href="{$site_root}/watch.php?id={$items.id}" class="d_block relative pp_wrap m_bottom_15">
					          <img src="{$site_root}/vidimgs/{$items.image}" alt="{$items.name}"  class="tr_all_hover">					          </a>
						      <!--description and price of product-->
						      <figcaption>
						        <h5 class="m_bottom_10"><a href="{$site_root}/watch.php?id={$items.id}" class="color_dark">{$items.name}</a></h5>
					          </figcaption>
					        </figure>
					      </div>
						  {/foreach}							
