 ...
 mysqli_free_result($title);
   $insertSQL = sprintf("INSERT INTO videos (added,login,featured,pass,visible, fname, name, `desc`, series, image, length, type,sub) VALUES (%s,%s,%s, %s, %s, %s, %s, %s, %s,%s,%s,%s,%s)",
	...
	GetSQLValueString($vvisible, "text",$GLOBALS['__Connect']),
                       ...
                       GetSQLValueString($items['something'], "text",$GLOBALS['__Connect']),
                       GetSQLValueString($items['something'], "text",$GLOBALS['__Connect']),
                       GetSQLValueString($items['group-title'], "text",$GLOBALS['__Connect']),
...
{
if(isset($items['url'])){
$insertSQL = sprintf("INSERT INTO live(name,fname,image,visible,display,price,series) VALUES(%s,%s,%s,%s,%s,%s,%s)",
...
if($row_user['active'] == 'true' or $row_user['role']  == 'Reseller') {
	...
	if(isset($_GET['accesscheck']))
	{
	$MM_redirectLoginSuccess = htmlspecialchars($_GET['accesscheck']);
	}
...
$MM_authorizedusers_db = "Reseller";
