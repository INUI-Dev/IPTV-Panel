...
<form method="get">Filter : 
  <label>time added: 
   <select name="psearch" id="psearch">
     {foreach $m3u_names as $items}<option value="{$items.name}">{$items.name}</option>{/foreach}
   </select>
  <input type="submit" name="go" id="go" value="Go" />
  <input type="hidden" name="pname" id="pname" value="add_live">
  </label>
  <input name="cate" type="hidden" id="cate" value="{$smarty.get.cate|escape}" />
</form></div><form method="post" enctype="multipart/form-data" id="form1" name="form1" action="?pname=add_live&psearch={$smarty.get.psearch|escape}&cate={$smarty.get.cate|escape}&pageNum={$pageNum}">
